...
const locationController = {
  ...
  create: async (req, res) => {
    try {
      const { name, latitude, longitude, address, city, state, landmarks, locationType } = req.body;
      const userId = req.user ? req.user.id : null;

      // Check if location already exists at these coordinates
      const existingLocation = await Location.findByCoordinates(latitude, longitude, 0.0001);
      if (existingLocation) {
        throw new ConflictError('A location already exists at these coordinates');
      }
      ...
      if (duplicateName) {
        throw new ConflictError(`Location "${name}" already exists in ${city}, ${state}`);
      }
  ...
  search: async (req, res) => {
    try {
      const { q, city, state, locationType, limit = 20, offset = 0 } = req.query;
      ...
      if (result.rows.length > 0) {
        await Promise.all(
          result.rows.slice(0, 5).map(location => location.incrementSearchCount())
        );
      }
  ...
  findNearby: async (req, res) => {
    try {
      const { lat, lng, radius = 10, limit = 20, locationType } = req.query;
      ...
      if (latitude < 4.0 || latitude > 14.0 || longitude < 2.5 || longitude > 15.0) {
        throw new ValidationError('Coordinates must be within Nigeria boundaries');
      }

      let locations = await Location.findNearby(latitude, longitude, radiusKm, maxLimit);
  ...
  update: async (req, res) => {
    try {
      const { id } = req.params;
      const { name, address, city, state, landmarks, locationType, isActive } = req.body;
      ...
      if (!canEdit) {
        throw new AuthorizationError('Insufficient permissions to edit this location');
      }
    ...
    try {
      ...
      const locations = await Location.findAll({
        ...
        attributes: [
          ...
          'city', 'state', 'locationType', 'searchCount', 'routeCount'
        ]
      });
  ...
  getByState: async (req, res) => {
    try {
      ...
      const locations = await Location.getByState(state, Math.min(parseInt(limit), 100));
  ...
  getByCity: async (req, res) => {
    try {
      ...
      const locations = await Location.getByCity(city, state, Math.min(parseInt(limit), 100));
  ...
  delete: async (req, res) => {
    try {
      ...
      if (!canDelete) {
        throw new AuthorizationError('Insufficient permissions to delete this location');
      }
    ...
    try {
      const stats = await Location.findAll({
        attributes: [
          ...
          [Location.sequelize.fn('AVG', Location.sequelize.col('searchCount')), 'avgSearches']
        ],
        where: { isActive: true },
        group: ['state'],
      ...
      const verifiedLocations = await Location.count({ where: { isActive: true, isVerified: true } });
      ...
      res.json({
        ...
        data: {
          ...
          byState: stats.map(stat => ({
            ...
            avgSearches: parseFloat(stat.dataValues.avgSearches || 0).toFixed(2)
          }))
        }
      });
