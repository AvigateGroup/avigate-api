...
const routeController = {
        ...
        try {
            ...
            if (existingRoute) {
                throw new ConflictError(
                    'A route between these locations already exists'
                )
            }
            ...
            for (let i = 0; i < stepNumbers.length; i++) {
                if (stepNumbers[i] !== i + 1) {
                    throw new ValidationError(
                        'Route steps must be sequential starting from 1'
                    )
                }
            }
            ...
            const route = await Route.create(
                {
                    ...
                    crowdsourcedData: {
                        ...
                        confidence: req.user
                            ? Math.min(req.user.reputationScore / 100, 5)
                            : 1,
                    },
                },
                { transaction }
            ...
            logger.info(
                `Route created from ${startLocation.name} to ${endLocation.name}`,
                {
                    routeId: route.id,
                    userId: req.user?.id,
    ...
    search: async (req, res) => {
        try {
            ...
            if (!from || !to) {
                throw new ValidationError(
                    'From and to location IDs are required'
                )
            }
            ...
            if (vehicleTypes && vehicleTypes.length > 0) {
                whereClause.vehicleTypes = {
                    [Op.overlap]: Array.isArray(vehicleTypes)
                        ? vehicleTypes
                        : [vehicleTypes],
                }
            }
            ...
            if (difficulty && difficulty.length > 0) {
                whereClause.difficulty = {
                    [Op.in]: Array.isArray(difficulty)
                        ? difficulty
                        : [difficulty],
                }
            }
            ...
            const routesWithMetadata = routes.map((route) => {
                ...
                routeData.metadata = {
                    ...
                    vehicleChanges: routeData.steps.filter(
                        (step, index) =>
                            index > 0 &&
                            step.vehicleType !==
                                routeData.steps[index - 1].vehicleType
                    ).length,
        ...
        try {
            ...
            if (!canEdit) {
                throw new AuthorizationError(
                    'Insufficient permissions to edit this route'
                )
            }
            ...
            if (Object.keys(updates).length > 0) {
                updates.crowdsourcedData = {
                    ...
                    lastUpdated: new Date(),
                    contributorCount:
                        (route.crowdsourcedData?.contributorCount || 0) + 1,
                }
            }
    ...
    submitFeedback: async (req, res) => {
        try {
            ...
            if (!routeStep) {
                throw new ValidationError(
                    'Route step does not belong to this route'
                )
            }
            ...
            if (req.user) {
                ...
                if (recentFeedback) {
                    throw new ConflictError(
                        'You have already provided feedback for this route step in the last 24 hours'
                    )
                }
            }
    ...
    getPopular: async (req, res) => {
        try {
            ...
            const routes = await Route.findAll({
                ...
                order: [
                    [
                        Route.sequelize.literal(
                            "(crowdsourced_data->>'feedbackCount')::int"
                        ),
                        'DESC',
                    ],
                    [
                        Route.sequelize.literal(
                            "(crowdsourced_data->>'confidence')::float"
                        ),
                        'DESC',
                    ],
                    ['createdAt', 'DESC'],
    ...
    delete: async (req, res) => {
        try {
            ...
            if (!canDelete) {
                throw new AuthorizationError(
                    'Insufficient permissions to delete this route'
                )
            }
        ...
        try {
            ...
            const routesByDifficulty = await Route.findAll({
                attributes: [
                    'difficulty',
                    [
                        Route.sequelize.fn('COUNT', Route.sequelize.col('id')),
                        'count',
                    ],
                ],
                where: { isActive: true },
            ...
            const routesByVehicleType = await Route.findAll({
                attributes: [
                    [
                        Route.sequelize.fn(
                            'UNNEST',
                            Route.sequelize.col('vehicleTypes')
                        ),
                        'vehicleType',
                    ],
            ...
            const avgFareRange = await Route.findOne({
                attributes: [
                    [
                        Route.sequelize.fn(
                            'AVG',
                            Route.sequelize.col('estimatedFareMin')
                        ),
                        'avgMinFare',
                    ],
                    [
                        Route.sequelize.fn(
                            'AVG',
                            Route.sequelize.col('estimatedFareMax')
                        ),
                        'avgMaxFare',
                    ],
                    [
                        Route.sequelize.fn(
                            'AVG',
                            Route.sequelize.col('estimatedDuration')
                        ),
                        'avgDuration',
                    ],
                ],
            ...
            res.json({
                ...
                data: {
                    ...
                    averages: {
                        minFare: parseFloat(
                            avgFareRange.avgMinFare || 0
                        ).toFixed(2),
                        maxFare: parseFloat(
                            avgFareRange.avgMaxFare || 0
                        ).toFixed(2),
                        duration:
                            parseFloat(avgFareRange.avgDuration || 0).toFixed(
                                0
                            ) + ' minutes',
                    },
                },
            ...
            res.json({
                ...
                data: {
                    ...
                    pagination: {
                        ...
                        offset: parseInt(offset),
                        hasMore:
                            routes.count > parseInt(offset) + parseInt(limit),
                    },
                },
            })
        } catch (error) {
