...
const totpController = {
    ...
    try {
      ...
      if (admin.totpEnabled) {
        return res.status(400).json({
          success: false,
          message: 'TOTP is already enabled. Disable first to regenerate.'
        });
      }
  ...
  enableTOTP: async (req, res) => {
    try {
      ...
      if (!admin.totpSecret) {
        return res.status(400).json({
          success: false,
          message: 'TOTP secret not found. Please generate a new secret first.'
        });
      }
      ...
      try {
        ...
        res.json({
          ...
          data: {
            backupCodes,
            message: 'Store these backup codes securely. They can only be used once each.'
          }
        });
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          backupCodesRemaining: admin.totpBackupCodes ? admin.totpBackupCodes.length : 0
        }
      });
  ...
  regenerateBackupCodes: async (req, res) => {
    try {
      ...
      logger.info(`TOTP backup codes regenerated for admin: ${admin.email}`);
