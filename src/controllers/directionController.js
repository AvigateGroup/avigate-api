...
const directionController = {
  ...
  create: async (req, res) => {
    try {
      ...
      if (!req.user) {
        throw new AuthenticationError('Authentication required to create directions');
      }
      ...
      if (!routeData || !routeData.steps || !Array.isArray(routeData.steps)) {
        throw new ValidationError('Route data must contain a valid steps array');
      }
      ...
      do {
        ...
        const existing = await UserDirection.findOne({ where: { shareCode } });
        if (!existing) break;
        attempts++;
      } while (attempts < 10);
      ...
      const completeDirection = await UserDirection.findByPk(direction.id, {
        include: [
  ...
  getByShareCode: async (req, res) => {
    try {
      ...
      const canAccess = direction.isPublic || 
      ...
      res.json({
        ...
        data: {
          ...
          pagination: {
            ...
            offset: parseInt(offset),
            hasMore: directions.count > (parseInt(offset) + parseInt(limit))
          }
        }
      });
  ...
  update: async (req, res) => {
    try {
      ...
      if (direction.createdBy !== req.user.id) {
        throw new AuthorizationError('You can only edit your own directions');
      }
      ...
      if (totalEstimatedFare) updates.totalEstimatedFare = totalEstimatedFare;
      if (totalEstimatedDuration) updates.totalEstimatedDuration = totalEstimatedDuration;
  ...
  delete: async (req, res) => {
    try {
      ...
      // Check permissions
      if (direction.createdBy !== req.user.id) {
        throw new AuthorizationError('You can only delete your own directions');
      }
  ...
  getStats: async (req, res) => {
    try {
      ...
      const publicDirections = await UserDirection.count({ where: { isPublic: true } });
      const totalUsage = await UserDirection.sum('usageCount');

      const topCreators = await UserDirection.findAll({
        attributes: [
          'createdBy',
          [UserDirection.sequelize.fn('COUNT', UserDirection.sequelize.col('id')), 'directionCount'],
          [UserDirection.sequelize.fn('SUM', UserDirection.sequelize.col('usageCount')), 'totalUsage']
        ],
        include: [{
          model: User,
          as: 'creator',
        ...
        order: [[UserDirection.sequelize.fn('COUNT', UserDirection.sequelize.col('id')), 'DESC']],
        limit: 5
      });

      const avgUsage = totalDirections > 0 ? (totalUsage / totalDirections).toFixed(2) : 0;
      ...
      res.json({
        ...
        data: {
          ...
          publicPercentage: totalDirections > 0 ? 
            ((publicDirections / totalDirections) * 100).toFixed(1) + '%' : '0%',
          topCreators: topCreators.map(creator => ({
            user: creator.creator,
            directionCount: parseInt(creator.dataValues.directionCount),
            totalUsage: parseInt(creator.dataValues.totalUsage || 0)
          }))
        }
      });
