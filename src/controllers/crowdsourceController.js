    ...
    submitRouteUpdate: async (req, res) => {
        try {
            ...
            const newCrowdsourcedData = {
                ...
                contributorCount:
                    (currentCrowdsourcedData.contributorCount || 0) + 1,
            ...
            if (req.user) {
                ...
                if (recentReport) {
                    throw new ConflictError(
                        'You have already reported fare for this route step in the last 6 hours'
                    )
                }
            }
        ...
        try {
            ...
            if (!req.user) {
                throw new AuthenticationError(
                    'Authentication required to suggest new routes'
                )
            }
            ...
            if (req.user.reputationScore < 50) {
                throw new AuthorizationError(
                    'Minimum reputation score of 50 required to suggest new routes'
                )
            }
            ...
            if (existingRoute) {
                throw new ConflictError(
                    'A route between these locations already exists. Consider updating the existing route instead.'
                )
            }
            ...
            logger.info(
                `New route suggested: ${startLocation.name} to ${endLocation.name}`,
                {
                    routeId: route.id,
                    userId: req.user.id,
                    confidence,
            ...
            res.status(201).json({
                success: true,
                message:
                    'New route suggestion submitted successfully. It will be reviewed before activation.',
                data: {
                    route: {
                        id: route.id,
        ...
        try {
            ...
            const routeUpdates = await Route.findAll({
                where: {
                    crowdsourcedData: {
                        [Op.contains]: {
                            updates: [{ contributorId: req.user.id }],
                        },
                    },
                },
                include: [
            ...
            res.json({
                ...
                data: {
                    stats,
                    contributions: {
                        fareReports: fareReports.slice(
                            0,
                            type === 'fares' ? parseInt(limit) : 5
                        ),
                        createdRoutes: createdRoutes.slice(
                            0,
                            type === 'routes' ? parseInt(limit) : 5
                        ),
                        routeUpdates: routeUpdates.slice(
                            0,
                            type === 'updates' ? parseInt(limit) : 5
                        ),
                    },
                },
            })
    ...
    getStats: async (req, res) => {
        try {
            ...
            res.status(error.statusCode || 500).json({
                success: false,
                message:
                    error.message || 'Failed to get crowdsourcing statistics',
                error: error.message,
            })
        }
    },
...
const calculateAggregatedData = (updates, updateType, newValue) => {
    ...
    switch (updateType) {
        case 'fare':
            const avgFare =
                recentUpdates.reduce((sum, update) => sum + update.value, 0) /
