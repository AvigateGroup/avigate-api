...
const nigerianPhoneRegex = /^(\+234|234|0)(70|80|81|90|91|80|81|70|90|91)[0-9]{8}$/;
...
const schemas = {
  ...
  latitude: Joi.number().min(NIGERIA_BOUNDS.lat.min).max(NIGERIA_BOUNDS.lat.max).required(),
  longitude: Joi.number().min(NIGERIA_BOUNDS.lng.min).max(NIGERIA_BOUNDS.lng.max).required(),
  ...
  // Nigerian states
  nigerianState: Joi.string().valid(
...
const routeValidators = {
  ...
  search: Joi.object({
    from: schemas.uuid,
    to: schemas.uuid,
    vehicleTypes: Joi.array().items(Joi.string().valid('bus', 'taxi', 'keke', 'okada', 'train')).optional(),
    maxFare: Joi.number().integer().min(0).optional(),
    maxDuration: Joi.number().integer().min(0).optional(),
    difficulty: Joi.array().items(Joi.string().valid('Easy', 'Medium', 'Hard')).optional(),
...
const crowdsourceValidators = {
  routeUpdate: Joi.object({
    ...
    updateType: Joi.string().valid('fare', 'duration', 'availability', 'condition').required(),
    newValue: Joi.alternatives().conditional('updateType', {
      switch: [
        ...
        { is: 'condition', then: Joi.string().valid('good', 'fair', 'poor') }
      ]
    }),
  ...
  fareReport: Joi.object({
    ...
    trafficCondition: Joi.string().valid('light', 'moderate', 'heavy').optional(),
    weatherCondition: Joi.string().valid('clear', 'rainy', 'cloudy').optional()
  }),
...
const landmarkValidators = {
  create: Joi.object({
    ...
    visibility: Joi.string().valid('High', 'Medium', 'Low').default('Medium')
  }),
