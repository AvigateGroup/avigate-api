...
class PushNotificationService {
  static async sendToUser(userId, notification) {
    ...
    try {
      ...
      const message = {
        ...
        android: {
          priority: notification.priority === PRIORITY_LEVELS.HIGH ? 'high' : 'normal',
          notification: {
  ...
  static async sendToMultipleUsers(userIds, notification) {
    ...
    logger.info(`Bulk push notification: ${successCount}/${userIds.length} successful`);
...
class NotificationService {
  ...
  static async sendNotification(userId, notification, channels = ['push']) {
    ...
    try {
      ...
      if (channels.includes('push')) {
        results.push = await PushNotificationService.sendToUser(userId, notification);
      }
      ...
      if (channels.includes('database')) {
        results.database = await this.storeNotification(userId, notification);
      }
  ...
  static async sendWelcomeNotification(userId) {
    ...
    return await this.sendNotification(userId, notification, ['push', 'database']);
  }
  ...
  static async sendContributionAcceptedNotification(userId, contributionData) {
    const notification = {
  ...
  static async sendBulkNotification(userIds, notification) {
    ...
    logger.info(`Bulk notification sent: ${successCount}/${userIds.length} successful`);
