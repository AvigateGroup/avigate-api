...
const routeService = {
            ...
            const enrichedRoutes = await Promise.all(
                routes.rows.map(async (route) => {
                    ...
                    const recentFeedback =
                        await routeService.getRouteFeedbackSummary(route.id)

                    // Calculate reliability score based on feedback
                    const reliabilityScore =
                        routeService.calculateReliabilityScore(recentFeedback)
    ...
    getRouteDetails: async (routeId) => {
        try {
            ...
            const feedbackSummary =
                await routeService.getRouteFeedbackSummary(routeId)

            // Calculate metrics
            const reliabilityScore =
                routeService.calculateReliabilityScore(feedbackSummary)
    ...
    getRouteFeedbackSummary: async (routeId) => {
        try {
            ...
            const averageRating =
                ...
                totalFeedbacks
            const averageFare =
                allFeedbacks.reduce((sum, fb) => sum + fb.actualFarePaid, 0) /
                totalFeedbacks
            const lastUpdated = Math.max(
                ...allFeedbacks.map((fb) => new Date(fb.createdAt).getTime())
            )
    ...
    calculateReliabilityScore: (feedbackSummary) => {
        ...
        const reliabilityScore =
            ratingScore * confidenceFactor + 50 * (1 - confidenceFactor)
    ...
    calculateRecentTrends: (feedbacks) => {
        ...
        // Calculate averages
        const olderAvgFare =
            olderFeedbacks.reduce((sum, fb) => sum + fb.actualFarePaid, 0) /
            olderFeedbacks.length
        const recentAvgFare =
            recentFeedbacks.reduce((sum, fb) => sum + fb.actualFarePaid, 0) /
            recentFeedbacks.length

        const olderAvgRating =
            olderFeedbacks.reduce((sum, fb) => sum + fb.rating, 0) /
            olderFeedbacks.length
        const recentAvgRating =
            recentFeedbacks.reduce((sum, fb) => sum + fb.rating, 0) /
            recentFeedbacks.length
        ...
        return {
            faretrend:
                Math.abs(fareChange) < fareThreshold
                    ? 'stable'
                    : fareChange > 0
                      ? 'increasing'
                      : 'decreasing',
            ratingTrend:
                Math.abs(ratingChange) < ratingThreshold
                    ? 'stable'
                    : ratingChange > 0
                      ? 'improving'
                      : 'declining',
        ...
        try {
            ...
            switch (updateType) {
                case 'fare':
                    ...
                    updates.estimatedFareMin = Math.min(
                        route.estimatedFareMin,
                        newValue
                    )
                    updates.estimatedFareMax = Math.max(
                        route.estimatedFareMax,
                        newValue
                    )
                    break
            ...
            crowdsourcedData.totalUpdates =
                (crowdsourcedData.totalUpdates || 0) + 1
            crowdsourcedData.averageConfidence =
                ((crowdsourcedData.averageConfidence || 3) *
                    (crowdsourcedData.totalUpdates - 1) +
                    confidence) /
                crowdsourcedData.totalUpdates

            if (comments) {
                crowdsourcedData.recentComments =
                    crowdsourcedData.recentComments || []
                ...
                crowdsourcedData.recentComments =
                    crowdsourcedData.recentComments.slice(0, 10)
            }
        ...
        try {
            ...
            if (allFeedbacks.length > 0) {
                ...
                allFeedbacks.forEach((fb) => {
                    ...
                    stats.vehicleTypeUsage[vehicle] =
                        (stats.vehicleTypeUsage[vehicle] || 0) + 1
                })
                ...
                allFeedbacks.forEach((fb) => {
                    ...
                    if (hour >= 5 && hour < 12) stats.timeOfDayUsage.morning++
                        ...
                        stats.timeOfDayUsage.afternoon++
                    else if (hour >= 17 && hour < 21)
                        stats.timeOfDayUsage.evening++
                    else stats.timeOfDayUsage.night++

                    // Day of week
                    stats.dayOfWeekUsage[dayOfWeek] =
                        (stats.dayOfWeekUsage[dayOfWeek] || 0) + 1
                })
            }
