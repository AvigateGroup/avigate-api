module.exports = (sequelize, DataTypes) => {
  const UserDirection = sequelize.define('UserDirection', {
      ...
      validate: {
        ...
        isValidRouteData(value) {
          ...
          if (!value.steps || !Array.isArray(value.steps) || value.steps.length === 0) {
            throw new Error('Route data must contain at least one step');
          }
          
          // Validate each step has required fields
          for (let i = 0; i < value.steps.length; i++) {
            const step = value.steps[i];
            if (!step.instructions || !step.vehicleType || !step.pickupPoint || !step.dropoffPoint) {
    ...
    category: {
      type: DataTypes.ENUM(
        'daily_commute', 'tourist_route', 'emergency_route', 
        'shopping_trip', 'airport_route', 'night_route', 
        'business_trip', 'other'
      ),
      defaultValue: 'other',
      allowNull: false
    ...
    hooks: {
      beforeCreate: (userDirection) => {
        ...
        if (!userDirection.shareCode) {
          userDirection.shareCode = UserDirection.generateShareCode();
        }
        ...
        if (userDirection.description) {
          userDirection.description = userDirection.description.trim();
        }
      },
      
      beforeUpdate: (userDirection) => {
        // Update lastAccessedAt when usage count changes
        if (userDirection.changed('usageCount') || userDirection.changed('viewCount')) {
  ...
  UserDirection.prototype.incrementUsage = async function(userId = null) {
    ...
    await this.save({ fields: ['usageCount', 'lastAccessedAt', 'accessHistory'] });
  };
  ...
  UserDirection.prototype.incrementView = async function(userId = null) {
    ...
    this.accessHistory = accessHistory;
    await this.save({ fields: ['viewCount', 'lastAccessedAt', 'accessHistory'] });
  };
  ...
  // Class methods
  UserDirection.findByShareCode = function(shareCode, includeInactive = false) {
    ...
    return UserDirection.findOne({
      ...
      include: [
        ...
        },
        {
          ...
          as: 'startLocation',
          attributes: ['id', 'name', 'latitude', 'longitude', 'address', 'city']
        },
        {
          model: sequelize.models.Location,
          as: 'endLocation',
  ...
  UserDirection.findByRoute = function(startLocationId, endLocationId, limit = 10) {
    return UserDirection.findAll({
      where: {
        startLocationId,
        endLocationId,
  ...
  UserDirection.getStatistics = async function() {
    const stats = await UserDirection.findOne({
      attributes: [
        ...
        [sequelize.fn('COUNT', sequelize.literal('CASE WHEN "isPublic" = true THEN 1 END')), 'publicDirections'],
        [sequelize.fn('COUNT', sequelize.literal('CASE WHEN "isActive" = true THEN 1 END')), 'activeDirections'],
        [sequelize.fn('COUNT', sequelize.literal('CASE WHEN "isFeatured" = true THEN 1 END')), 'featuredDirections'],
