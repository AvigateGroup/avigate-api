module.exports = (sequelize, DataTypes) => {
                ...
                validate: {
                    ...
                    isGreaterThanMin(value) {
                        if (this.fareMin && value < this.fareMin) {
                            throw new Error(
                                'Maximum fare must be greater than or equal to minimum fare'
                            )
                        }
                    },
                },
    ...
    RouteStep.prototype.getFareRange = function () {
        ...
        return {
            ...
            average:
                this.fareMin && this.fareMax
                    ? Math.round((this.fareMin + this.fareMax) / 2)
                    : this.fareMin || this.fareMax || 0,
        }
    }

    RouteStep.prototype.addCrowdsourceFareReport = async function (
        fare,
        userId,
        confidence = 3
    ) {
        if (!this.crowdsourcedFare) {
            this.crowdsourcedFare = {
                reports: [],
                average: null,
                lastUpdated: null,
            }
        ...
        if (this.crowdsourcedFare.reports.length > 20) {
            this.crowdsourcedFare.reports =
                this.crowdsourcedFare.reports.slice(-20)
        }

        // Calculate weighted average
        const totalWeight = this.crowdsourcedFare.reports.reduce(
            (sum, report) => sum + report.confidence,
            0
        )
        const weightedSum = this.crowdsourcedFare.reports.reduce(
            (sum, report) => sum + report.fare * report.confidence,
            0
        )

        this.crowdsourcedFare.average =
    ...
    RouteStep.prototype.addCrowdsourceDurationReport = async function (
        duration,
        userId,
        confidence = 3
    ) {
        if (!this.crowdsourcedDuration) {
            this.crowdsourcedDuration = {
                reports: [],
                average: null,
                lastUpdated: null,
            }
        ...
        if (this.crowdsourcedDuration.reports.length > 20) {
            this.crowdsourcedDuration.reports =
                this.crowdsourcedDuration.reports.slice(-20)
        }

        // Calculate weighted average
        const totalWeight = this.crowdsourcedDuration.reports.reduce(
            (sum, report) => sum + report.confidence,
            0
        )
        const weightedSum = this.crowdsourcedDuration.reports.reduce(
            (sum, report) => sum + report.duration * report.confidence,
            0
        )

        this.crowdsourcedDuration.average =
            totalWeight > 0 ? Math.round(weightedSum / totalWeight) : null
    ...
    RouteStep.prototype.isPeakTime = function (time = new Date()) {
        ...
        return (
            (currentTime >= this.peakHours.morning.start &&
                currentTime <= this.peakHours.morning.end) ||
            (currentTime >= this.peakHours.evening.start &&
                currentTime <= this.peakHours.evening.end)
        )
    }
    ...
    RouteStep.prototype.toJSON = function () {
        ...
        step.estimatedFareWithPeak = this.getEstimatedFareWithPeak(
            step.isCurrentlyPeak
        )
    ...
    RouteStep.validateStepSequence = function (steps) {
        ...
        for (let i = 0; i < sortedSteps.length - 1; i++) {
            if (
                sortedSteps[i].toLocationId !==
                sortedSteps[i + 1].fromLocationId
            ) {
                return {
                    isValid: false,
