module.exports = (sequelize, DataTypes) => {
                ...
                validate: {
                    isValidVehicleTypes(value) {
                        ...
                        if (invalidTypes.length > 0) {
                            throw new Error(
                                `Invalid vehicle types: ${invalidTypes.join(', ')}`
                            )
                        }
                    },
                },
    ...
    Route.prototype.addCrowdsourceData = async function (type, data) {
        ...
        if (this.crowdsourcedData[reportKey]) {
            ...
            if (this.crowdsourcedData[reportKey].length > 50) {
                this.crowdsourcedData[reportKey] =
                    this.crowdsourcedData[reportKey].slice(-50)
            }
        }
    ...
    Route.prototype.getCrowdsourceAverage = function (type) {
        if (
            !this.crowdsourcedData ||
            !this.crowdsourcedData[`${type}Reports`]
        ) {
            return null
        }
    ...
    Route.prototype.getEstimatedFareRange = function () {
        return {
            ...
            average: Math.round(
                (this.estimatedFareMin + this.estimatedFareMax) / 2
            ),
        }
    }
    ...
    // Class methods
    Route.findByLocations = function (
        startLocationId,
        endLocationId,
        options = {}
    ) {
    ...
    Route.searchRoutes = function (options = {}) {
        ...
        if (maxDuration)
            whereClause.estimatedDuration = { [Op.lte]: maxDuration }
        if (difficulty && Array.isArray(difficulty))
            whereClause.difficulty = { [Op.in]: difficulty }
        ...
        return Route.findAll({
            where: {
                [Op.and]: [
                    { isActive: true },
                    sequelize.literal(
                        `"vehicleTypes"::jsonb ? '${vehicleType}'`
                    ),
                ],
            },
