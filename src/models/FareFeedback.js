module.exports = (sequelize, DataTypes) => {
                ...
                validate: {
                    ...
                    isNotFuture(value) {
                        if (new Date(value) > new Date()) {
                            throw new Error(
                                'Date of travel cannot be in the future'
                            )
                        }
                    },
                    isRecentEnough(value) {
                        ...
                        if (new Date(value) < oneYearAgo) {
                            throw new Error(
                                'Date of travel cannot be more than one year ago'
                            )
                        }
                    },
                },
    ...
    FareFeedback.prototype.getDurationComparison = function (
        estimatedDuration
    ) {
    ...
    FareFeedback.getAverageByRouteStep = function (routeStepId, options = {}) {
        ...
        return FareFeedback.findOne({
            ...
            attributes: [
                [
                    sequelize.fn('AVG', sequelize.col('actualFarePaid')),
                    'avgFare',
                ],
                [
                    sequelize.fn('MIN', sequelize.col('actualFarePaid')),
                    'minFare',
                ],
                [
                    sequelize.fn('MAX', sequelize.col('actualFarePaid')),
                    'maxFare',
                ],
                [
                    sequelize.fn('AVG', sequelize.col('actualDuration')),
                    'avgDuration',
                ],
            ...
            attributes: [
                ...
                [
                    sequelize.fn(
                        'COUNT',
                        sequelize.literal(
                            'CASE WHEN "isVerified" = true THEN 1 END'
                        )
                    ),
                    'verifiedFeedback',
                ],
                [
                    sequelize.fn(
                        'COUNT',
                        sequelize.literal(
                            'CASE WHEN "isFlagged" = true THEN 1 END'
                        )
                    ),
                    'flaggedFeedback',
                ],
                ...
                [
                    sequelize.fn('AVG', sequelize.col('confidenceScore')),
                    'avgConfidence',
                ],
            ],
            raw: true,
